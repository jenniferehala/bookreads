...
class Author:
    ...
    def get_user_authors(cls, data):
        query = "SELECT * FROM users LEFT JOIN books ON users.id = user_id WHERE users.id = %(id)s"
    ...
    @classmethod
    def save(cls, data):
        query = "INSERT INTO cars (author_fname, author_lname, user_id) VALUES (%(author_fname)s, %(authorlname)s,%(user_id)s);"
